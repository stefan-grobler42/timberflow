...
export const TendersPage = () => {
  ...
  const defaultColumnOrder = [
    'name', 'description', 'fileLink', 'closingDate', 'customer', 'quoteNo', 'totalValueExcl'
  ];
  ...
  const loadViews = (): GridView[] => {
    try {
      const saved = localStorage.getItem('tenders-views');
      if (saved) {
  ...
  const [selection] = useState(
    new Selection({
      onSelectionChanged: () => {
        const selected = selection.getSelection()[0] as Tender | undefined;
  ...
  useEffect(() => {
    applyFilters();
  }, [tenders, searchText, currentView, sortColumn, isSortedDescending]);
  ...
  const applyFilters = () => {
    ...
    if (currentView.filters && currentView.filters.length > 0) {
      filtered = filtered.filter((tender) => {
        ...
        for (let i = 0; i < currentView.filters.length; i++) {
          ...
          const fieldValue = tender[filter.field as keyof Tender];
    ...
    if (sortColumn) {
      filtered.sort((a, b) => {
        let aValue: any = a[sortColumn as keyof Tender];
        let bValue: any = b[sortColumn as keyof Tender];
  ...
  const handleRowDoubleClick = (tender: Tender) => {
  ...
  const handleDelete = () => {
    if (selectedTender) {
      setTenderToDelete(selectedTender);
  ...
  const confirmDelete = async () => {
    if (tenderToDelete) {
      try {
        await tenderService.delete(tenderToDelete.id);
  ...
  const exportToExcel = (tendersToExport: Tender[]) => {
    const exportData = tendersToExport.map((tender) => ({
      'Name': tender.name || '',
      'Description': tender.description || '',
      'File Link': tender.fileLink || '',
      'Closing Date': tender.closingDate ? new Date(tender.closingDate).toLocaleDateString() : '',
      'Customer': tender.customer || '',
      'Quote No': tender.quoteNo || '',
      'Total Value (Excl)': tender.totalValueExcl || 0,
    }));
  ...
  const handleExportSelected = () => {
    const selected = selection.getSelection() as Tender[];
  ...
  const handleSaveView = (view: GridView) => {
    ...
    setCurrentView(view);
    localStorage.setItem('tenders-views', JSON.stringify(updatedViews));
  };
  ...
  const availableFields = [
    ...
    { key: 'fileLink', name: 'File Link', type: 'text' as const },
    { key: 'customer', name: 'Customer', type: 'text' as const },
    { key: 'quoteNo', name: 'Quote No', type: 'text' as const },
  ];
  ...
  const allColumns: IColumn[] = [
    ...
    },
    {
      ...
      maxWidth: 150,
      isResizable: true,
      isSorted: sortColumn === 'fileLink',
      isSortedDescending: sortColumn === 'fileLink' && isSortedDescending,
      onColumnClick: onColumnClick,
    },
    {
      key: 'closingDate',
      ...
      maxWidth: 120,
      isResizable: true,
      isSorted: sortColumn === 'closingDate',
      isSortedDescending: sortColumn === 'closingDate' && isSortedDescending,
      onColumnClick: onColumnClick,
      onRender: (item: Tender) => <Text>{item.closingDate ? new Date(item.closingDate).toLocaleDateString() : '-'}</Text>,
    },
    {
      key: 'customer',
      name: 'Customer',
    ...
    },
    {
      ...
      maxWidth: 120,
      isResizable: true,
      isSorted: sortColumn === 'quoteNo',
      isSortedDescending: sortColumn === 'quoteNo' && isSortedDescending,
      onColumnClick: onColumnClick,
    },
    {
      key: 'totalValueExcl',
      ...
      maxWidth: 150,
      isResizable: true,
      isSorted: sortColumn === 'totalValueExcl',
      isSortedDescending: sortColumn === 'totalValueExcl' && isSortedDescending,
      onColumnClick: onColumnClick,
      onRender: (item: Tender) => <Text>{item.totalValueExcl ? `R ${item.totalValueExcl.toLocaleString()}` : '-'}</Text>,
    },
  ];
  ...
  const commandBarItems: ICommandBarItemProps[] = [
    ...
    },
    {
      ...
      onClick: () => document.getElementById('tenders-import-input')?.click(),
    },
  ];
    ...
    return (
      <div style={{ height: '100vh', overflow: 'auto' }}>
        <TenderForm
          ...
          onDelete={selectedTender ? () => handleDelete() : undefined}
        />
      </div>
    );
  }
  ...
  return (
    <Stack tokens={{ childrenGap: 16 }}>
      <Text variant="xxLarge">Tenders</Text>

      <CommandBar items={commandBarItems} farItems={commandBarFarItems} />

      <input
        id="tenders-import-input"
        type="file"
        accept=".xlsx,.xls"
        style={{ display: 'none' }}
      ...
      {loading ? (
        <Stack horizontalAlign="center" tokens={{ padding: 40 }}>
          <Spinner size={SpinnerSize.large} label="Loading tenders..." />
        </Stack>
      ) : (
