...
import { pickingTeamService } from '../services/millenniumServices';
import type { PickingTeam } from '../types/millennium';
import type { GridView, GridFilter } from '../types/gridView';
import { PickingTeamForm } from '../components/PickingTeamForm';
import { DeleteDialog } from '../components/DeleteDialog';
import { ViewManager } from '../components/ViewManager';
import { FilterBuilder } from '../components/FilterBuilder';
import * as XLSX from 'xlsx';

export const PickingTeamsPage = () => {
  const [pickingTeams, setPickingTeams] = useState<PickingTeam[]>([]);
  const [filteredPickingTeams, setFilteredPickingTeams] = useState<PickingTeam[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedPickingTeam, setSelectedPickingTeam] = useState<PickingTeam | undefined>();
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
  const [pickingTeamToDelete, setPickingTeamToDelete] = useState<PickingTeam | undefined>();
  ...
  const defaultVisibleColumns: { [key: string]: boolean } = {
    ...
    description: true,
    teamLeader: true,
    averageTimePerM3: true,
  };

  const defaultColumnOrder = [
    'name', 'description', 'teamLeader', 'averageTimePerM3'
  ];

  const defaultView: GridView = {
    id: 'default-view',
    name: 'All Picking Teams',
    isDefault: true,
    entityType: 'pickingteam',
  ...
  const loadViews = (): GridView[] => {
    try {
      const saved = localStorage.getItem('pickingteams-views');
      if (saved) {
  ...
  const [selection] = useState(
    new Selection({
      onSelectionChanged: () => {
        const selected = selection.getSelection()[0] as PickingTeam | undefined;
        setSelectedPickingTeam(selected);
      },
    })
  );
  ...
  useEffect(() => {
    applyFilters();
  }, [pickingTeams, searchText, currentView, sortColumn, isSortedDescending]);
  ...
  const loadPickingTeams = async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to load picking teams');
    } finally {
      setLoading(false);
    }
  };
  ...
  const applyFilters = () => {
    let filtered = [...pickingTeams];

    if (searchText) {
      const search = searchText.toLowerCase();
      filtered = filtered.filter((pickingTeam) =>
        Object.values(pickingTeam).some((value) =>
    ...
    if (currentView.filters && currentView.filters.length > 0) {
      filtered = filtered.filter((pickingTeam) => {
        ...
        for (let i = 0; i < currentView.filters.length; i++) {
          ...
          const fieldValue = pickingTeam[filter.field as keyof PickingTeam];
    ...
    if (sortColumn) {
      filtered.sort((a, b) => {
        let aValue: any = a[sortColumn as keyof PickingTeam];
        let bValue: any = b[sortColumn as keyof PickingTeam];
    ...
    setFilteredPickingTeams(filtered);
  };

  const handleNew = () => {
    setSelectedPickingTeam(undefined);
    setIsFormOpen(true);
  };

  const handleEdit = () => {
    if (selectedPickingTeam) {
      setIsFormOpen(true);
    }
  };

  const handleRowDoubleClick = (pickingTeam: PickingTeam) => {
    setSelectedPickingTeam(pickingTeam);
    setIsFormOpen(true);
  };

  const handleDelete = () => {
    if (selectedPickingTeam) {
      setPickingTeamToDelete(selectedPickingTeam);
      setIsDeleteDialogOpen(true);
    }
  };

  const confirmDelete = async () => {
    if (pickingTeamToDelete) {
      try {
        await pickingTeamService.delete(pickingTeamToDelete.id);
        setIsDeleteDialogOpen(false);
        setPickingTeamToDelete(undefined);
        await loadPickingTeams();
      } catch (err) {
        setError(err instanceof Error ? err.message : 'Failed to delete picking team');
      }
    }
  };

  const exportToExcel = (pickingTeamsToExport: PickingTeam[]) => {
    const exportData = pickingTeamsToExport.map((pickingTeam) => ({
      'Name': pickingTeam.name || '',
      'Description': pickingTeam.description || '',
      'Team Leader': pickingTeam.teamLeader || '',
      'Average Time /m³': pickingTeam.averageTimePerM3 || 0,
    }));

    const ws = XLSX.utils.json_to_sheet(exportData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Picking Teams');
    XLSX.writeFile(wb, `PickingTeams_Export_${new Date().toISOString().split('T')[0]}.xlsx`);
  };

  const handleExportSelected = () => {
    const selected = selection.getSelection() as PickingTeam[];
  ...
  const handleExportAll = () => {
    exportToExcel(filteredPickingTeams);
    setIsExportDialogOpen(false);
  };
  ...
  const handleSaveView = (view: GridView) => {
    ...
    setCurrentView(view);
    localStorage.setItem('pickingteams-views', JSON.stringify(updatedViews));
  };
  ...
  const availableFields = [
    ...
    { key: 'teamLeader', name: 'Team Leader', type: 'text' as const },
    { key: 'averageTimePerM3', name: 'Average Time /m³', type: 'number' as const },
  ];
  ...
  const allColumns: IColumn[] = [
    ...
    },
    {
      ...
      maxWidth: 200,
      isResizable: true,
      isSorted: sortColumn === 'teamLeader',
      isSortedDescending: sortColumn === 'teamLeader' && isSortedDescending,
      onColumnClick: onColumnClick,
    },
    {
      key: 'averageTimePerM3',
      name: 'Average Time /m³',
      minWidth: 120,
      maxWidth: 150,
      isResizable: true,
      isSorted: sortColumn === 'averageTimePerM3',
      isSortedDescending: sortColumn === 'averageTimePerM3' && isSortedDescending,
      onColumnClick: onColumnClick,
      onRender: (item: PickingTeam) => <Text>{item.averageTimePerM3 ? item.averageTimePerM3.toFixed(2) : '-'}</Text>,
    },
  ];
  ...
  const commandBarItems: ICommandBarItemProps[] = [
    ...
    },
    {
      ...
      onClick: () => document.getElementById('pickingteams-import-input')?.click(),
    },
  ];
  ...
  if (isFormOpen) {
    return (
      <div style={{ height: '100vh', overflow: 'auto' }}>
        <PickingTeamForm
          pickingTeam={selectedPickingTeam}
          onDismiss={() => {
            setIsFormOpen(false);
            setSelectedPickingTeam(undefined);
          }}
          ...
          onDelete={selectedPickingTeam ? () => handleDelete() : undefined}
        />
      </div>
    );
  }
  ...
  return (
    <Stack tokens={{ childrenGap: 16 }}>
      <Text variant="xxLarge">Picking Teams</Text>

      <CommandBar items={commandBarItems} farItems={commandBarFarItems} />

      <input
        id="pickingteams-import-input"
        type="file"
        accept=".xlsx,.xls"
        style={{ display: 'none' }}
      ...
      {loading ? (
        <Stack horizontalAlign="center" tokens={{ padding: 40 }}>
          <Spinner size={SpinnerSize.large} label="Loading picking teams..." />
        </Stack>
      ) : (
        <div style={{ overflowX: 'auto', overflowY: 'visible' }}>
          <DetailsList
            items={filteredPickingTeams}
            columns={columns}
            selection={selection}
            selectionMode={1}
      ...
      <DeleteDialog
        ...
        title="Delete Picking Team"
        message={`Are you sure you want to delete ${pickingTeamToDelete?.name}?`}
      ...
      {isExportDialogOpen && (
        ...
        >
          <Stack tokens={{ childrenGap: 16 }} styles={{ root: { marginTop: 16 } }}>
            ...
            />
            <DefaultButton
              text={`Export All (${filteredPickingTeams.length})`}
