...
export const SuppliersPage = () => {
  ...
  const [selectedItem, setSelectedItem] = useState<SupplierItem | undefined>();
  ...
  const loadViews = (): GridView[] => {
    try {
      const saved = localStorage.getItem('suppliers-views');
      if (saved) {
  ...
  const [selection] = useState(
    new Selection({
      onSelectionChanged: () => {
        const selected = selection.getSelection()[0] as SupplierItem | undefined;
  ...
  const applyFilters = () => {
    ...
    if (currentView.filters && currentView.filters.length > 0) {
      filtered = filtered.filter((item) => {
        ...
        for (let i = 0; i < currentView.filters.length; i++) {
          ...
          const fieldValue = item[filter.field as keyof SupplierItem];
    ...
    if (sortColumn) {
      filtered.sort((a, b) => {
        let aValue: any = a[sortColumn as keyof SupplierItem];
        let bValue: any = b[sortColumn as keyof SupplierItem];
  ...
  const handleNew = () => {
    console.log('New supplier - form not implemented yet');
  };

  const handleEdit = () => {
    if (selectedItem) {
      console.log('Edit supplier - form not implemented yet');
    }
  };

  const handleRowDoubleClick = (item: SupplierItem) => {
  ...
  const handleSaveView = (view: GridView) => {
    ...
    setCurrentView(view);
    localStorage.setItem('suppliers-views', JSON.stringify(updatedViews));
  };
  ...
  return (
    <Stack styles={{ root: { padding: 20, height: '100%' } }}>
      <Stack horizontal horizontalAlign="space-between" verticalAlign="center" styles={{ root: { marginBottom: 20 } }}>
        <Text variant="xxLarge">Suppliers</Text>
