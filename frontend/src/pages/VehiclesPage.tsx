...
export const VehiclesPage = () => {
  ...
  const defaultVisibleColumns: { [key: string]: boolean } = {
    ...
    approvedDriver: true,
    cofInOrder: true,
    licenseRenewalDate: false,
  };

  const defaultColumnOrder = [
    'name', 'registrationNumber', 'yearModel', 'make', 'model',
    'approvedDriver', 'cofInOrder', 'licenseRenewalDate'
  ];
  ...
  const loadViews = (): GridView[] => {
    try {
      const saved = localStorage.getItem('vehicles-views');
      if (saved) {
  ...
  const [selection] = useState(
    new Selection({
      onSelectionChanged: () => {
        const selected = selection.getSelection()[0] as Vehicle | undefined;
  ...
  useEffect(() => {
    applyFilters();
  }, [vehicles, searchText, currentView, sortColumn, isSortedDescending]);
  ...
  const applyFilters = () => {
    ...
    if (currentView.filters && currentView.filters.length > 0) {
      filtered = filtered.filter((vehicle) => {
        ...
        for (let i = 0; i < currentView.filters.length; i++) {
          ...
          const fieldValue = vehicle[filter.field as keyof Vehicle];
    ...
    if (sortColumn) {
      filtered.sort((a, b) => {
        let aValue: any = a[sortColumn as keyof Vehicle];
        let bValue: any = b[sortColumn as keyof Vehicle];
  ...
  const exportToExcel = (vehiclesToExport: Vehicle[]) => {
    const exportData = vehiclesToExport.map((vehicle) => ({
      ...
      'Make': vehicle.make || '',
      'Model': vehicle.model || '',
      'Approved Driver': vehicle.approvedDriver || '',
      'COF In Order': vehicle.cofInOrder ? 'Yes' : 'No',
      'License Renewal Date': vehicle.licenseRenewalDate ? new Date(vehicle.licenseRenewalDate).toLocaleDateString() : '',
    }));
  ...
  const handleExportSelected = () => {
    const selected = selection.getSelection() as Vehicle[];
  ...
  const handleSaveView = (view: GridView) => {
    ...
    setCurrentView(view);
    localStorage.setItem('vehicles-views', JSON.stringify(updatedViews));
  };
  ...
  const availableFields = [
    ...
    { key: 'registrationNumber', name: 'Registration Number', type: 'text' as const },
    { key: 'yearModel', name: 'Year Model', type: 'text' as const },
    { key: 'make', name: 'Make', type: 'text' as const },
    { key: 'model', name: 'Model', type: 'text' as const },
    { key: 'approvedDriver', name: 'Approved Driver', type: 'text' as const },
    { key: 'cofInOrder', name: 'COF In Order', type: 'boolean' as const },
  ];
  ...
  const allColumns: IColumn[] = [
    ...
    },
    {
      ...
      maxWidth: 150,
      isResizable: true,
      isSorted: sortColumn === 'registrationNumber',
      isSortedDescending: sortColumn === 'registrationNumber' && isSortedDescending,
      onColumnClick: onColumnClick,
    },
    {
      key: 'yearModel',
      ...
      maxWidth: 120,
      isResizable: true,
      isSorted: sortColumn === 'yearModel',
      isSortedDescending: sortColumn === 'yearModel' && isSortedDescending,
      onColumnClick: onColumnClick,
    },
    {
      key: 'make',
      ...
      maxWidth: 150,
      isResizable: true,
      isSorted: sortColumn === 'make',
      isSortedDescending: sortColumn === 'make' && isSortedDescending,
      onColumnClick: onColumnClick,
    },
    {
      key: 'model',
      ...
      maxWidth: 150,
      isResizable: true,
      isSorted: sortColumn === 'model',
      isSortedDescending: sortColumn === 'model' && isSortedDescending,
      onColumnClick: onColumnClick,
    },
    {
      key: 'approvedDriver',
      name: 'Approved Driver',
      fieldName: 'approvedDriver',
      minWidth: 120,
      maxWidth: 150,
      isResizable: true,
      isSorted: sortColumn === 'approvedDriver',
      isSortedDescending: sortColumn === 'approvedDriver' && isSortedDescending,
      onColumnClick: onColumnClick,
    },
    {
      key: 'cofInOrder',
      ...
      maxWidth: 120,
      isResizable: true,
      isSorted: sortColumn === 'cofInOrder',
      isSortedDescending: sortColumn === 'cofInOrder' && isSortedDescending,
      onColumnClick: onColumnClick,
      onRender: (item: Vehicle) => <Text>{item.cofInOrder ? 'Yes' : 'No'}</Text>,
    },
    {
      ...
      maxWidth: 150,
      isResizable: true,
      isSorted: sortColumn === 'licenseRenewalDate',
      isSortedDescending: sortColumn === 'licenseRenewalDate' && isSortedDescending,
      onColumnClick: onColumnClick,
      onRender: (item: Vehicle) => <Text>{item.licenseRenewalDate ? new Date(item.licenseRenewalDate).toLocaleDateString() : '-'}</Text>,
    },
  ];
  ...
  const commandBarItems: ICommandBarItemProps[] = [
    ...
    },
    {
      ...
      onClick: () => document.getElementById('vehicles-import-input')?.click(),
    },
  ];
  ...
  return (
    <Stack tokens={{ childrenGap: 16 }}>
      <Text variant="xxLarge">Vehicles</Text>

      <CommandBar items={commandBarItems} farItems={commandBarFarItems} />

      <input
        id="vehicles-import-input"
        type="file"
        accept=".xlsx,.xls"
        style={{ display: 'none' }}
      ...
      {loading ? (
        <Stack horizontalAlign="center" tokens={{ padding: 40 }}>
          <Spinner size={SpinnerSize.large} label="Loading vehicles..." />
        </Stack>
      ) : (
