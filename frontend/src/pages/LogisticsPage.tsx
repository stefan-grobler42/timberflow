...
export const LogisticsPage = () => {
  ...
  const [filteredLogistics, setFilteredLogistics] = useState<Logistics[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedLogistics, setSelectedLogistics] = useState<Logistics | undefined>();
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
  const [logisticsToDelete, setLogisticsToDelete] = useState<Logistics | undefined>();
  ...
  const defaultColumnOrder = ['deliveryNo', 'description', 'plannedLoadDate', 'newLoadCompleted'];
  ...
  const loadViews = (): GridView[] => {
    try {
      const saved = localStorage.getItem('logistics-views');
      if (saved) {
  ...
  useEffect(() => {
    applyFilters();
  }, [logistics, searchText, currentView, sortColumn, isSortedDescending]);
    ...
    if (currentView.filters && currentView.filters.length > 0) {
      filtered = filtered.filter((item) => {
        ...
        for (let i = 0; i < currentView.filters.length; i++) {
          ...
          const fieldValue = item[filter.field as keyof Logistics];
    ...
    if (sortColumn) {
      filtered.sort((a, b) => {
        ...
        if (comparison === 0 && sortColumn !== 'deliveryNo') {
          const aName = a.deliveryNo || '';
          const bName = b.deliveryNo || '';
  ...
  const handleNew = () => {
    setSelectedLogistics(undefined);
    setIsFormOpen(true);
  };
  ...
  const handleRowDoubleClick = (item: Logistics) => {
  ...
  const handleDelete = () => {
    if (selectedLogistics) {
      setLogisticsToDelete(selectedLogistics);
  ...
  const confirmDelete = async () => {
    if (logisticsToDelete) {
      try {
        ...
        setLogisticsToDelete(undefined);
        await loadLogistics();
      } catch (err) {
        setError(err instanceof Error ? err.message : 'Failed to delete logistics');
      }
    }
  };

  const exportToExcel = (itemsToExport: Logistics[]) => {
    const exportData = itemsToExport.map((item) => ({
      'Delivery No': item.deliveryNo || '',
      'Description': item.description || '',
      'Planned Load Date': item.plannedLoadDate ? new Date(item.plannedLoadDate).toLocaleDateString() : '',
      'Completed': item.newLoadCompleted ? 'Yes' : 'No',
    }));
  ...
  const handleExportSelected = () => {
    const selected = selection.getSelection() as Logistics[];
  ...
  const handleExportAll = () => {
    exportToExcel(filteredLogistics);
    setIsExportDialogOpen(false);
  };
  ...
  const handleSaveView = (view: GridView) => {
    ...
    setCurrentView(view);
    localStorage.setItem('logistics-views', JSON.stringify(updatedViews));
  };
  ...
  const availableFields = [
    ...
    { key: 'newLoadCompleted', name: 'Completed', type: 'boolean' as const },
  ];
  ...
  const allColumns: IColumn[] = [
    {
      ...
      isSortedDescending: sortColumn === 'deliveryNo' && isSortedDescending,
    ...
    },
    {
      ...
      isSortedDescending: sortColumn === 'plannedLoadDate' && isSortedDescending,
      onColumnClick: onColumnClick,
    ...
    },
    {
      ...
      isSortedDescending: sortColumn === 'newLoadCompleted' && isSortedDescending,
      onColumnClick: onColumnClick,
  ...
  const commandBarItems: ICommandBarItemProps[] = [
    ...
    },
    {
      ...
      onClick: () => document.getElementById('logistics-import-input')?.click(),
    },
  ];
    ...
    return (
      <div style={{ height: '100vh', overflow: 'auto' }}>
        <LogisticsForm
          ...
          onDelete={selectedLogistics ? () => handleDelete() : undefined}
        />
      </div>
    );
  }
  ...
  return (
    <Stack tokens={{ childrenGap: 16 }}>
      <Text variant="xxLarge">Logistics</Text>

      <CommandBar items={commandBarItems} farItems={commandBarFarItems} />

      <input
        id="logistics-import-input"
        type="file"
        accept=".xlsx,.xls"
        style={{ display: 'none' }}
      ...
      {loading ? (
        <Stack horizontalAlign="center" tokens={{ padding: 40 }}>
          <Spinner size={SpinnerSize.large} label="Loading logistics..." />
        </Stack>
      ) : (
      ...
      <DeleteDialog
        ...
        itemName={logisticsToDelete?.deliveryNo || ''}
